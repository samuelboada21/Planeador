...
export const asignCompetencesUpdate = async (materiaId, nuevasCompetencias, t, res) => {
    const errorMessage = 'El formato de las competencias por asignar no es correcto';
    ...
    if (!nuevasCompetencias.every(competencia => typeof competencia === 'number')) {
        res.status(400);
        throw new Error(errorMessage);
    }
    ...
    const competenciasActualesIds = competenciasActuales.map(competencia => competencia.competencia_id);
    // Encontrar las competencias a eliminar (las que están en competenciasActuales pero no en nuevasCompetencias)
    const competenciasAEliminar = competenciasActualesIds.filter(competencia => !nuevasCompetencias.includes(competencia));
    // Encontrar las competencias a agregar (las que están en nuevasCompetencias pero no en competenciasActuales)
    const competenciasAAgregar = nuevasCompetencias.filter(competencia => !competenciasActualesIds.includes(competencia));
...
export const asignTipoEvidencias = async (instrumentoId, tipos, t, res) => {
    const errorMessage = 'El formato de los tipos de evidencia por asignar no es correcto'
...
export const asignTipoUpdate = async (instrumentoId, nuevosTipos, t, res) => {
    ...
    const tiposAEliminar = tiposActualesIds.filter(tipo => !nuevosTipos.includes(tipo));
  
    // Encontrar los tipos a agregar (los que están en nuevosTipos pero no en tiposActuales)
    const tiposAAgregar = nuevosTipos.filter(tipo => !tiposActualesIds.includes(tipo));

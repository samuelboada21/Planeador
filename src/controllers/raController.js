  ...
  try {
    ...
    if (!categoria) {
      return res
        .status(400)
        .json({
          error: "No se encontró la categoría asociada a la competencia",
        });
    }
    ...
    res
      .status(200)
      .json({ message: "Resultado de Aprendizaje creado exitosamente" });
  } catch (err) {
...
const updateResultado = async (req, res, next) => {
  ...
  try {
    const [resultado, resultadoExist, competencia_exist, cantidadRA] =
      await Promise.all([
    ...
    res
      .status(200)
      .json({ message: "Resultado de aprendizaje actualizado correctamente" });
  } catch (err) {
  ...
  try {
    ...
    if (!resultado) {
      ...
      return res
        .status(400)
        .json({
          error: "No se encontro el resultado de aprendizaje especificado",
        });
    }
    // Eliminamos el RA
